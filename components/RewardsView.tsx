...
const RewardsView: React.FC<RewardsViewProps> = ({ rewards, redeemedRewards, coins, onAddReward, onDeleteReward, onRedeemReward, onEditReward, onToggleUsed }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 space-y-6">
      <div className="bg-slate-200 dark:bg-slate-700 p-1 rounded-xl flex items-center space-x-1 max-w-lg mx-auto">
      ...
      {activeTab === 'store' && (
        <div className="space-y-8 animate-pop-in">
          {/* Add Reward Form */}
          <div className="p-6 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900">
            <h3 className="text-xl font-extrabold text-slate-700 dark:text-slate-200 mb-4 flex items-center gap-2">
            ...
            <form onSubmit={handleAddReward} className="flex flex-col sm:flex-row items-end gap-4">
                <div className="flex-grow w-full">
                    <label htmlFor="reward-name" className="block text-sm font-bold text-slate-600 dark:text-slate-400 mb-1">
                    ...
                    <input
                        ...
                        required
                        className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 bg-white dark:bg-slate-800 dark:text-white font-semibold"
                    />
                </div>
                <div className="w-full sm:w-40">
                    <label htmlFor="reward-cost" className="block text-sm font-bold text-slate-600 dark:text-slate-400 mb-1">
                        Custo em moedas
                    </label>
                    <input
                        id="reward-cost"
          ...
          <div>
            {rewards.length === 0 ? (
              <div className="text-center py-10 px-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                <p className="text-slate-500 dark:text-slate-400">Nenhuma recompensa cadastrada ainda.</p>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Use o formulário acima para adicionar a primeira!</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {rewards.sort((a, b) => a.cost - b.cost).map(reward => {
                  ...
                  return (
                    <div key={reward.id} className={`p-5 rounded-xl border transition-all duration-300 flex flex-col justify-between ${canAfford ? 'bg-white dark:bg-slate-800 shadow-md border-slate-200 dark:border-slate-600' : 'bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-700'}`}>
                      <div>
                        <div className="flex justify-between items-start gap-2">
                            <h4 className={`font-extrabold text-lg break-words flex-grow ${canAfford ? 'text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-500'}`}>{reward.name}</h4>
                            <div className="flex-shrink-0 flex items-center">
                                <button 
                                    onClick={() => onEditReward(reward)}
                                    className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 p-1 rounded-full transition-colors"
                                ...
                                <button 
                                    ...
                                    className="text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 p-1 rounded-full transition-colors"
                        ...
                        <div className={`flex items-center gap-2 mt-2 font-bold ${canAfford ? 'text-amber-600 dark:text-amber-500' : 'text-slate-400 dark:text-slate-600'}`}>
                      ...
                      <button
                        ...
                        disabled={!canAfford}
                        className="w-full mt-4 font-bold py-2 px-4 rounded-lg transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none focus:outline-none focus:ring-2 focus:ring-offset-2 bg-green-600 text-white shadow-lg shadow-green-500/30 hover:bg-green-700 focus:ring-green-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:text-slate-500 dark:disabled:text-slate-500"
                      >
                        Resgatar
                      </button>
                    </div>
      ...
      {activeTab === 'history' && (
        <div className="space-y-4 animate-pop-in">
          ...
          {redeemedRewards.length === 0 ? (
            <div className="text-center py-10 px-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                <p className="text-slate-500 dark:text-slate-400">Nenhuma recompensa foi resgatada ainda.</p>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Continue completando missões para juntar moedas!</p>
            </div>
          ) : (
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                {redeemedRewards
                    ...
                    .map(reward => (
                        <div key={reward.uniqueId} className={`p-4 rounded-xl border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-all duration-300 ${reward.used ? 'bg-slate-100 dark:bg-slate-900 opacity-70 border-slate-200 dark:border-slate-800' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}>
                            <div className="flex-grow">
                                <p className={`font-bold text-slate-800 dark:text-slate-200 ${reward.used ? 'line-through' : ''}`}>{reward.name}</p>
                            ...
                            <div className="flex-shrink-0 flex items-center gap-4 w-full sm:w-auto">
                                <div className="flex items-center gap-2 font-bold text-amber-600 dark:text-amber-500 bg-amber-100 dark:bg-amber-900/40 px-3 py-1 rounded-full">
                                ...
                                <button
                                    ...
                                    className={`w-full sm:w-auto flex items-center justify-center gap-2 font-bold py-2 px-4 rounded-lg transition duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                                        reward.used 
                                        ? 'bg-slate-300 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-400 dark:hover:bg-slate-600 focus:ring-slate-400' 
